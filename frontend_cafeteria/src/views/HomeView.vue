...
onMounted(async () => {
  try {
    ...
    if (esAdmin.value) {
      ...
      promesas.push(axios.get('/productos'), axios.get('/ventas'))
    }

    const respuestas = await Promise.all(promesas)

    if (esAdmin.value) {
      const [productos, clientes, ventas, categorias] = respuestas
      ...
      const [productos, ventas] = respuestas
      totalProductos.value = productos.data.length
      totalVentas.value = ventas.data.length
      totalRecaudado.value = ventas.data.reduce((acc: number, v: any) => acc + Number(v.total), 0)
      ultimasVentas.value = ventas.data.slice(0, 5)
...
<template>
  <div class="w-full px-6 py-6 mx-auto">
    <div class="mb-6">
      ...
      >
        ...
        <p style="margin: 2px 0 0; font-size: 0.85rem; color: #a98a66">
          {{ esAdmin ? 'Panel de administración' : 'Panel de ventas' }} — Cafetería El Buen Gusto
        </p>
      </div>
    </div>
    ...
    >
      <div
        v-if="esAdmin"
        style="background: white; border: 1px solid #ede2d3; border-radius: 1rem; padding: 1rem"
      >
        <div style="display: flex; justify-content: space-between; align-items: flex-start">
          <div>
            ...
            <h5 style="font-size: 1.5rem; font-weight: 600; margin: 0; color: #4a2c2a">
              {{ totalCategorias }}
            </h5>
    ...
    <div class="flex flex-wrap -mx-3">
          ...
          <div class="p-4">
            <div class="overflow-x-auto">
              <table class="w-full mb-0 border-collapse">
                ...
                <tbody>
                  <tr
                    v-for="venta in ultimasVentas.slice(0, esAdmin ? 5 : 3)"
                    :key="venta.id"
                    style="border-bottom: 1px solid #f0e7d9"
                  >
                    <td style="padding: 12px 14px; font-size: 0.85rem; color: #b5a48e">
                      {{ venta.id }}
                    </td>
                    <td
                      style="
      ...
      <div v-if="esAdmin" class="w-full max-w-full px-3 lg:w-5/12 mb-6">
        <div
          style="
            background: white;
            border: 1px solid #ede2d3;
        ...
        >
          <div style="padding: 1rem 1.25rem; border-bottom: 1px solid #f0e7d9">
            <div class="flex justify-between items-center">
              ...
              <RouterLink
                to="/productos"
                style="font-size: 0.85rem; color: #6f4e37; font-weight: 600"
              >
                Ver todos →
              </RouterLink>
            </div>
          ...
          <div class="p-4">
            <ul class="flex flex-col pl-0 mb-0">
              ...
              >
                <div class="flex items-center">
                  ...
                  <div class="flex flex-col">
                    <h6 style="margin: 0; font-size: 0.85rem; font-weight: 600; color: #4a2c2a">
                      {{ producto.nombre }}
                    </h6>
                ...
                <div class="text-right">
                  <span style="font-size: 0.85rem; font-weight: 700; color: #3b6d11"
                    >Bs. {{ producto.precio }}</span
                  >
                  <br />
                  <span style="font-size: 0.75rem; color: #b5a48e"
              ...
              <li
                ...
                style="padding: 1rem; text-align: center; font-size: 0.85rem; color: #b5a48e"
              >
